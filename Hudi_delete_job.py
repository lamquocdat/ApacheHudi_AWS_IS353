...
args = getResolvedOptions(
    sys.argv, ['JOB_NAME',
               ...
               'HUDI_PRECOMB_KEY', 'HUDI_RECORD_KEY', 
               'DELETES',
               'TARGET_S3_PATH']
)
...
if(args['DELETES']):
    for value in json.loads(args['DELETES']):
        current_condition = (col(value[0]) == value[1])
        if filter_condition is None:
            filter_condition = current_condition
        else:
            filter_condition = filter_condition | current_condition

S3bucket_node3_df = S3bucket_node3_df.filter(filter_condition)
